  ...
  <div class="lstm-prediction">
    ...
    <div v-else>
      <div class="prediction-summary">
        ...
        <div class="signal" :class="prediction.signal === '買入' ? 'buy' : 'sell'">
          {{ prediction.signal }}
        </div>
      </div>
      ...
      <div class="future-predictions">
        ...
        <table>
          ...
          <tbody>
            <tr v-for="(pred, index) in prediction.future_predictions" :key="index">
              <td>{{ pred.date }}</td>
              <td 
                :class="index === 0 ? 'current' : 
                      pred.price > prediction.future_predictions[index-1].price ? 'up' : 'down'">
  ...
  methods: {
    ...
    async renderChart() {
      try {
        ...
        const futureDates = this.prediction.future_predictions.map(d => d.date);
        const futurePrices = this.prediction.future_predictions.map(d => d.price);
        ...
        const traceCurrent = {
          x: [historicalDates.length > 0 ? historicalDates[historicalDates.length - 1] : futureDates[0]],
        ...
        const allPrices = [...historicalPrices, ...futurePrices, this.prediction.current_price];
        ...
        const layout = {
          ...
          shapes: [
            ...
            {
              type: 'line',
              x0: historicalDates.length > 0 ? historicalDates[historicalDates.length - 1] : futureDates[0],
              y0: minPrice,
              x1: historicalDates.length > 0 ? historicalDates[historicalDates.length - 1] : futureDates[0],
              y1: maxPrice,
              line: {
          ...
          annotations: [
            ...
            // 添加未來預測標註
            {
              ...
              xref: 'x',
              yref: 'y',
              text: `預測: $${futurePrices[futurePrices.length - 1].toFixed(2)}`,
              showarrow: true,
              arrowhead: 2,
              arrowsize: 1,
              arrowwidth: 2,
